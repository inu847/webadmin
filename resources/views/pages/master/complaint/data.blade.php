...
<div class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            <a href="{{ route('complaint.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
        </div>
    </div>
</div>
...
@section('modal')
    @include('pages.master.complaint.form')
@endsection
...
@section('customjs')
            ...
            methods: {
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.get("{{ url('/complaint/delete') }}" +'/'+ id)
                                .then((response) => {
                                    let self = this;
    ...
    <script type="text/javascript">
        ...
        function loadView(page = null) {
            preloadContent();
            if (page == null) {
                var url = "{{ url('/complaint/data') }}";
            } else {
                var url = "{{ url('/complaint') }}" + "/" + page;
            }
            var obj = new Object();
        ...
        function setPrimary(image,id){
            $.ajax({
                url: "{{ url('/complaint/primaryimage') }}" +'/'+ id +'/'+ image,
                type: 'GET',
                dataType: 'html',
            })
            .done(function(data) {
        ...
        function deleteImage(image,id,position){
            Swal.fire({
              ...
              if (result.value) {
                $.ajax({
                    url: "{{ url('/complaint/deleteimage') }}" +'/'+ id +'/'+ image +'/'+ position,
                    type: 'GET',
                    dataType: 'html',
                })
                .done(function(data) {
