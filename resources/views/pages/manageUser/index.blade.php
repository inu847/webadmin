        ...
        <div class="tabs-animation">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div id="tableData" class="main-card mb-3 card" style="min-height: 250px;">
                                ...
                                <tbody>
                                    @foreach ($users as  $user)
                                        <tr>
                                            <!-- <td>
                                                @if ($user->photo)
                                                    <img src="{{ asset('storage/'.$user->photo )}}" alt="" width="100px" width="100px">
                                                @else
                                                    photo not found
                                                @endif
                                            </td> -->
                                            ...
                                            <td>{{ Date::fullDate($user->created_at) }}</td>
                                            <td>{{ $user->name }} {{ optional($detail->catalog)->count() }}</td>
                                            <td>{{ $user->email }} {{ optional($detail->member)->count() }}</td>
                                            <td>{{ $user->phone }}</td>
                                            {{-- <td>{{ $user->number_catalog }}</td>
                                            <td>{{ $user->level }}</td>
                                            <td>{{ $user->active }}</td>
                                            <td>{{ $user->user_show }}</td>
                                            <td>{{ $user->legitimate }}</td>
                                            <td>{{ $user->owner }}</td>
                                            <td>{{ $user->parent_id }}</td>
                                            <td>{{ $user->catalog }}</td> --}}
                                            <td>
                                                <a href="javascript:void(0)" data-id="{{ $user->id }}" class="detaillink btn btn-shadow btn-primary">Detail</a>
                                                <a href="javascript:void(0)" data-id="{{ $user->id }}" class="editlink btn btn-shadow btn-info">Edit</a>
                                                <a href="javascript:void(0)" data-id="{{ $user->id }}" class="deletelink btn btn-shadow btn-danger">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            methods: {
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.delete("{{ url('/manage-user') }}" + "/" + id)
                                .then((response) => {
