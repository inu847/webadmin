    ...
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            @if(!empty($request['searchfield']))
                <a href="{{ route('account-foodcourt.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
            @endif
        </div>
    </div>
</div>
...
@section('customjs')
            ...
            methods: {
                ...
                showForm: function (action, id = null) {
                    ...
                    if (action == "create") {
                        ...
                        axios
                            .get("{{ route('account-foodcourt.create') }}")
                            .then((response) => {
                                var data = response.data;
                                $("#modalForm").modal("show");
                                $("#contentForm").html(data);
                        ...
                        axios
                            .get("{{ url('/account-foodcourt') }}" +"/" + id+"/edit")
                            .then((response) => {
                                var data = response.data;
                                $("#modalForm").modal("show");
                                $("#contentForm").html(data);
                ...
                confirmDialog: function (id,active) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.delete("{{ url('/account-foodcourt') }}" +'/'+ id)
                                .then((response) => {
                                    let self = this;
    ...
    <script type="text/javascript">
        ...
        function loadView(page = null) {
            preloadContent();
            if (page == null) {
                var url = "{{ url('/account-foodcourt/data') }}";
            } else {
                var url = "{{ url('/account-foodcourt') }}" + "/" + page;
            }
            var obj = new Object();
