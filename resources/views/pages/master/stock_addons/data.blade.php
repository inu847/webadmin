...
<div class="app-page-title">
    <div class="page-title-wrapper">
        <div class="page-title-heading">
            ...
            <div>
                Manage {{ (Session::get('catalogsession')=='All')?'Warehouse':getData::getCatalogSession('catalog_title') }} Addon Stock
        ...
        <div class="page-title-actions">
            <a href="{{ route('stock-addons.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
        </div>
    </div>
</div>
...
{{-- @section('modal')
    @include('pages.master.stock_addons.form')
@endsection --}}
...
@section('customjs')
            ...
            methods: {
                    ...
                    if (action == "create") {
                        ...
                        axios
                            ...
                            .then((response) => {
                                ...
                                $("#modalForm").modal('show');
                                $("#id").val(data.data.id);
                                $("#items_name").val(data.data.items_name);
                                $("#item_unit").val(data.data.item_unit);
                                $("#modalForm").modal('show');
                                afterpreloader();
                            })
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.get("{{ url('/stock-addons/delete') }}" + "/" + id)
                                .then((response) => {
                                    let self = this;
    ...
    <script type="text/javascript">
        ...
        function loadView(page = null) {
            preloadContent();
            if (page == null) {
                var url = "{{ url('/stock-addons/data') }}";
            } else {
                var url = "{{ url('/stock-addons') }}" + "/" + page;
            }
            var obj = new Object();
