            ...
            el.on('keypress', function(e) {
              if(e.keyCode === 13) {
                ...
                $(e.target).parents('.dd3-content').children('.dd-edit-box').first().slideToggle(50, function() {
                  ...
                  category_id = edtBox.find('input[name="category_id"]').first().val(),
                  subcategory_id = edtBox.find('input[name="subcategory_id"]').first().val(),
                  item_id = edtBox.find('input[name="item_id"]').first().val(),
                  item = edtBox.parents(opt.itemNodeName).first();
                  item.data('title', title).data('http', http).data('category_id', category_id).data('subcategory_id', subcategory_id).data('item_id', item_id);
                  spn.add(rmvBtn).slideToggle(50);
                });
              }
            })
          },
            createNewListItem: function(id, title, http, category_id, subcategory_id, item_id) {
              ...
              subcategory_id = typeof subcategory_id !== 'undefined' ? subcategory_id : '',
              ...
              blueprint.find('input[name="category_id"]').first().val(category_id);
              blueprint.data('category_id', category_id);
              blueprint.find('input[name="subcategory_id"]').first().val(subcategory_id);
              blueprint.data('subcategory_id', subcategory_id);
              blueprint.find('input[name="item_id"]').first().val(item_id);
              blueprint.data('item_id', item_id);
            ...
            deserialize: function(data, override) {
              ...
              var processItem = function(i, pref) {
                if(i.children) {
                  ...
                  item = _this.createNewListItem(i.id, i.title, i.http, i.category_id, i.subcategory_id, i.item_id);
                  pref.append(item);
                  item.append(cref);
                  ...
                  var item = _this.createNewListItem(i.id, i.title, i.http, i.category_id, i.subcategory_id, i.item_id);
                  pref.append(item);
                }
              }
