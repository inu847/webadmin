    ...
    <table class="align-middle mb-0 table table-borderless table-striped table-hover">
        <thead>
            <tr>
                ...
                <th class="pt-4 pb-4 text-center" nowrap>Voucher Type</th>
                <th class="pt-4 pb-4 text-right" nowrap>Value</th>
                <th class="pt-4 pb-4" nowrap>Owner</th>
                <th class="pt-4 pb-4 text-center" nowrap>Total Voucher Used</th>
        ...
        <tbody>
            ...
            <tr>
                <form class="mb-0" id="delete_form_{{ $value['id'] }}" action="{{ route('voucher.destroy', $value['id']) }}" method="post">
                    @csrf
                    @method('delete')
                        ...
                        <div class="widget-content p-0">
                            <div class="widget-content-wrapper">
                                <div class="widget-content-left flex2">
                                    <div class="widget-heading">{{ $value['voucher_code'] }}</div>
                    ...
                    <td class="text-right" nowrap>
                        {{ number_format($value['voucher_nominal']) }}
                    </td>
                    <td nowrap>
                        {{ $value['voucher_owner'] }}
                    </td>
                    <td class="text-center" nowrap>
                        {{ getData::countAllVoucher($value['voucher_code']) }}
                    </td>
                    <td class="text-center" nowrap>
                        <div role="group" class="btn-group-sm btn-group btn-group-toggle">
                            <a href="javascript:void(0)" class="btn btn-shadow btn-primary" onclick="voucherUsed('{{ $value['voucher_code'] }}')">Usage List</a>
