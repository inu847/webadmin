                ...
                <div class="table-responsive">
                    <table class="align-middle mb-0 table table-borderless table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <td>{{ number_format($detail->minimal_bisa_cicilan)}}</td>
                        </tr>
                        <tr>
                            <th>Maksimal bisa cicilan</th>
                            <td>{{ number_format($detail->maksimal_bisa_cicilan)}}</td>
                        </tr>
                        <tr>
                            <th>Keterangan</th>
                    ...
                    <table class="align-middle mb-0 table table-borderless table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($metodepembayaran as $item)
                                <tr>
                                    <td><img src="{{ "storage/".$item->logo }}" alt="" width="100px"></td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->code }}</td>
                                    <td>
                                        @if ($detail->active == 1)
...
@section('customjs')
    ...
    <script>
        function create() {
            ...
            $.ajax({
                url: "{{ route('create.MetodePembayaran', [$detail->id]) }}", 
                type: "GET", 
            })
            .done(function(data) {
            ...
            methods: {
                showForm: function (action, id = null) {
                    ...
                    if (action == "edit") {
                        ...
                        axios
                            .get("{{ url('/editMetodePembayaran') }}" +"/" + id +"/edit"  )
                            .then((response) => {
                        ...
                        $("#wrappassword").css("display", "none");
                        axios
                            .get("{{ url('/editMetodePembayaran') }}" +"/" + id  )
                            .then((response) => {
                                var data = response.data;
                                $("#modalForm").modal("show");
                                $("#contentForm").html(data);
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.delete("{{ url('destroyMetodePembayaran') }}" + "/" + id)
                                .then((response) => {
