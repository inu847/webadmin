                ...
                <form id="myForm" @submit.prevent="submitForm" method="post" onsubmit="return false;" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="position-relative form-group">
                            ...
                            <select id="item_id" name="item_id" class="custom-select" v-on:change="loadItem()">
                                <option value="">Select</option>
                                @foreach($material as $vmaterial)
                                    <option value="{{ $vmaterial['itemid'] }}">{{ $vmaterial['items_name'] }}</option>
                                @endforeach
                            </select>
	...
	new Vue({
	    ...
	    methods: {
	        submitForm: function (e) {
	            ...
	            if($("#id").val() > 0){
	                var action = "{{ url('/addons/updateserving/'.$item['id']) }}";
	            }else{
	                var action = "{{ url('/addons/addserving/'.$item['id']) }}";
	            }
