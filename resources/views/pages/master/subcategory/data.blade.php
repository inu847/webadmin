...
<div class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            <a href="{{ route('subcategory.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
        </div>
    </div>
</div>
...
@section('modal')
    @include('pages.master.subcategory.form')
@endsection
...
@section('customjs')
            ...
            methods: {
                showForm: function (action, id = null) {
                    ...
                    if (action == "create") {
                        ...
                        $("#myForm")[0].reset();
                        $("input:checkbox[name*=catalogs]").prop( "checked", false );
                        ...
                        axios
                            ...
                            .then((response) => {
                                ...
                                $("#subcategory_color").val(data.subcategory_color);
                                $('.simpleColorDisplay').css({'background':data.subcategory_color,'width':'25px','height':'25px','position':'absolute','right':'5px','top':'-35px','border-radius':'50px'});

                                $("input:checkbox[name*=catalogs]").prop( "checked", false );
                                $.each(catalog, function(index, value) {
                                    $("#catalog"+value).prop( "checked", true );
                                });
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.get("{{ url('/subcategory/delete') }}" +'/'+ id)
                                .then((response) => {
                                    let self = this;
    ...
    <script type="text/javascript">
        ...
        function loadView(page = null) {
            preloadContent();
            if (page == null) {
                var url = "{{ url('/subcategory/data') }}";
            } else {
                var url = "{{ url('/subcategory') }}" + "/" + page;
            }
            var obj = new Object();
