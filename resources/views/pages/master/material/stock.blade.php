...
<div class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            <a href="{{ route('material.index') }}" class="btn-shadow btn btn-success btn-sm"><i class="icon lnr-arrow-left"></i> Back</a>
            <a href="{{ url('/material/stock/'.$item['id']) }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
        </div>
    </div>
</div>
...
@section('modal')
@include('pages.master.material.formstock')
@endsection
        ...
        methods: {
            showForm: function (action, id = null) {
                ...
                if (action == "create") {
                    ...
                    axios.get("{{ url('/material/detail') }}" +'/'+ id)
                        .then((response) => {
            ...
            confirmDialog: function (item,addon) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        preloadContent();
                        axios.get("{{ url('/material/delete/stock') }}" + "/" + item + "/" + addon)
                            .then((response) => {
                                let self = this;
...
<script type="text/javascript">
    ...
    function loadView(page = null) {
        preloadContent();
        if (page == null) {
            var url = "{{ url('/material/stock') }}" + "/{{ $item['id'] }}";
        } else {
            var url = "{{ url('/material/stock') }}" + "/" + page;
        }
        var obj = new Object();
