    	...
    	try {
    	    DB::transaction(function () use ($request) {
                ...
                if($request->hasFile('image')){
                    ...
                    // Tutorial::where('id',$id)->update($array_one);
                    ...
                    $url = Storage::disk('s3')->url($thumbs.'/'.$filenametostore);
                    $img_insert['image'] = $url;
                    Tutorial::where('id',$id)->update($img_insert);
                }
    	    });
    	 }
    	catch(\Exception $e) {
    ...
    public static function update_data($request){
        try {
            DB::transaction(function () use ($request) {
                ...
                $query = Tutorial::where('id',$data['id'])->first();
                ...
                Tutorial::where('id',$data['id'])->update($array_one);
            });
         }
        catch(\Exception $e) {
            return false;
    ...
    public static function delete_data($id){
        try {
            DB::transaction(function () use ($id) {
            	$query = Tutorial::where('id',$id)->first();
