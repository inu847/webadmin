                ...
                <div class="modal-content">
                    ...
                    <div id="modalContent" class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="position-relative form-group">
                                    <label>Handler (PIC) <sup class="text-danger">* (Required)</sup></label>
                                    <input type="text" id="handler" name="handler" class="form-control"/>
                                    <div v-if="formErrors['handler']" class="errormsg alert alert-danger mt-1">
                                      @{{ formErrors['handler'][0] }}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="position-relative form-group">
                                    ...
                                    <input type="text" id="handled_on" name="handled_on" class="form-control"/>
                                    <div v-if="formErrors['handled_on']" class="errormsg alert alert-danger mt-1">
                                      @{{ formErrors['handled_on'][0] }}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="position-relative form-group">
                            <label for="myLabel" class="">Job Result Description</label>
                            <textarea id="job_result" name="job_result" class="form-control" rows="10"></textarea>
                            <div v-if="formErrors['job_result']" class="errormsg alert alert-danger mt-1">
                              @{{ formErrors['job_result'][0] }}
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
    ...
    new Vue({
        ...
        methods: {
            submitForm: function (e) {
                ...
                if($("#id").val() > 0){
                  var action = "{{ route('user_service.update',0) }}";
                  var put = form.querySelector('input[name="_method"]').value;
                }else{
                  var action = "{{ route('user_service.store') }}";
                  var put = '';
                }
                ...
                datas.append("handler", $("#handler").val());
                datas.append("handled_on", $("#handled_on").val());
                datas.append("lunas", $("#lunas").val());
                datas.append("job_result", CKEDITOR.instances['job_result'].getData());
            ...
            isChecked:function(addons) {
                $(document).on('show.bs.modal', '#modalForm', function () {
                    setTimeout(function(){ 
                        $.ajax({
                            url: "{{ url('/user_service/checkaddons') }}"+'/'+$("#id").val()+'/'+addons,
                            type: 'GET',
                        })
                        .done(function(data) {
                            if(data == 1){
