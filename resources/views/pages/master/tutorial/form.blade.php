                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="position-relative form-group">
                                    ...
                                    <label>https://drive.google.com/uc?export=download&id=<b class="text-danger">XXXXXXXX</b></label>
                                    
                                    <div v-if="formErrors['video']" class="errormsg alert alert-danger mt-1">
                                      @{{ formErrors['video'][0] }}
                                    </div>
                                </div>
                            </div>
    ...
    new Vue({
        ...
        methods: {
            submitForm: function (e) {
                ...
                if($("#id").val() > 0){
                  var action = "{{ route('tutorial.update',0) }}";
                  var put = form.querySelector('input[name="_method"]').value;
                }else{
                  var action = "{{ route('tutorial.store') }}";
                  var put = '';
                }
            ...
            isChecked:function(addons) {
                $(document).on('show.bs.modal', '#modalForm', function () {
                    setTimeout(function(){ 
                        $.ajax({
                            url: "{{ url('/tutorial/checkaddons') }}"+'/'+$("#id").val()+'/'+addons,
                            type: 'GET',
                        })
                        .done(function(data) {
                            if(data == 1){
