...
<div class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            <a href="{{ route('voucher.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
        </div>
    </div>
</div>
...
@section('customjs')
            ...
            methods: {
                ...
                confirmDialog: function (id) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            preloadContent();
                            axios.get("{{ url('/voucher/delete') }}" +'/'+ id)
                                .then((response) => {
                                    let self = this;
    ...
    <script type="text/javascript">
        function voucherUsed(code=null){
            preloader();
            $("#titleModal").html("List "+code+" Usage");
            $.ajax({
                url: "{{ url('/voucher/usagevoucher') }}"+'/'+code,
                type: 'GET',
            })
            .done(function(data) {
                $("#contentForm").html(data);
        ...
        function loadView(page = null) {
            preloadContent();
            if (page == null) {
                var url = "{{ url('/voucher/data') }}";
            } else {
                var url = "{{ url('/voucher') }}" + "/" + page;
            }
            var obj = new Object();
