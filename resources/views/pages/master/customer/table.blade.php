    ...
    <table class="align-middle mb-0 table table-borderless table-striped table-hover">
        <thead>
            <tr>
                ...
                <th class="pt-4 pb-4" nowrap>Action</th>
            </tr>
        </thead>
        <tbody>
            @foreach($getData as $key=>$value)
            <tr>
                <form class="mb-0" id="delete_form_{{ $value['id'] }}" action="{{ route('customer.destroy', $value['id']) }}" method="post">
                    @csrf
                    @method('delete')
                        ...
                        <div class="widget-content p-0">
                            <div class="widget-content-wrapper">
                                <div class="widget-content-left flex2">
                                    <div class="widget-heading">{{ $value['customer_name'] }}</div>
                    ...
                    <td nowrap>
                        <button type="button" onclick="detail({{ $value['customer_phone'] }})" class="btn btn-info">Show Detail</button>
                    </td>
                </form>
            </tr>
            @endforeach
