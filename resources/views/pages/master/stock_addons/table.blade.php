    ...
    <table class="align-middle mb-0 table table-borderless table-striped table-hover">
        <thead>
            <tr>
                ...
                <th class="pt-4 pb-4 " nowrap>Addon Name</th>
        ...
        <tbody>
            ...
            <tr>
                <form class="mb-0" id="delete_form_{{ $value['id'] }}" action="{{ route('stock-addons.destroy', $value['id']) }}" method="post">
                    @csrf
                    @method('delete')
                    ...
                    --}}
                    @if((Session::get('catalogsession')!='All'))
                        <td class="text-right" nowrap>{{ myFunction::formatNumber(getData::countStockBranchUsed($value['id'])) }}</td>
                    @endif
                    <td class="text-right" nowrap>
                    ...
                    <td class="text-center" nowrap>
                        <div role="group" class="btn-group-sm btn-group btn-group-toggle">
                            @if((Session::get('catalogsession')!='All'))
                            <a href="{{ url('/stock-addons/stock/'.$value['id']) }}" class="btn btn-shadow btn-primary">
                                Manage Stock
                            </a>
                            @endif
                            {{--
