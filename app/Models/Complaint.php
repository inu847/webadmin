    ...
    public static function save_data($request){
    	try {
    	    DB::transaction(function () use ($request) {
    	        $id = myFunction::id('complaints','id');
                $data=$request->all();
                $var=new Complaint;
                $var->id=$id;
                ...
                if($request->hasFile('image')){
                    ...
                    // Complaint::where('id',$id)->update($array_one);
                    ...
                    Complaint::where('id',$id)->update($img_insert);
    ...
    public static function update_data($request){
        try {
            DB::transaction(function () use ($request) {
                ...
                $query = Complaint::where('id',$data['id'])->first();
                ...
                Complaint::where('id',$data['id'])->update($array_one);
            });
         }
        catch(\Exception $e) {
            return false;
    ...
    public static function delete_data($id){
        try {
            DB::transaction(function () use ($id) {
            	$query = Complaint::where('id',$id)->first();
