    ...
    <div class="app-page-title">
        <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
                <a href="{{ route('menu.index') }}" class="btn-shadow btn btn-info btn-sm"><i class="icon lnr-sync"></i> Refresh</a>
            </div>
        </div>
    </div>
                                ...
                                <tbody>
                                    @foreach ($menus as $key => $menu)
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ $menu->name }}</td>
                                            <td>{{ $menu->active ? 'Active' : 'Not Active' }}</td>
                                            <td>
                                                <a href="{{ route('menu.show', [$menu->id]) }}" data-id="{{ $menu->id }}" class="detaillink btn btn-shadow btn-primary">Detail</a>
...
<div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div id="modalSize" class="modal-dialog modal-sm" role="document">
        ...
        new Vue({
            ...
            methods: {
                showForm: function (action, id = null) {
                    ...
                    if (action == "create") {
                        $("#titleModal").html("Create New Category");
                        $.ajax({
                            url: "{{ url('/menu/create') }}", 
                            type: "GET", 
                        })
